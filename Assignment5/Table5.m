...
Inputs:
    dmrsType (string/ char array): type of DMRS used- either "single" or 
        "double"
    configurationType (string/ char array): DMRS configuration type, either
        "Type1" or "Type2"
Outputs: 
...
function [ldash, ports] = Table5(dmrsType, configurationType)

    dmrsTypeCol = {'single', 'double'};

    ldashCol = {0; [0,1]};

    if configurationType == "Type1"
        typeCol = {1000:1003; 1000:1007};
    elseif configurationType == "Type2"
        typeCol = {1000:1005; 1000:1011};
    end

    table5 = table(ldashCol, typeCol, 'RowNames', dmrsTypeCol);

    rowType = table5(dmrsType, :);
